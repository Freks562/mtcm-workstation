...
router.post('/', rateLimiter, honeypot, verifyCaptcha, async (req, res) => {
  ...
  // Log the submission
  console.log(`[CONTACT] New submission: ${JSON.stringify({ formData, metadata })}`);

  // Send emails
  try {
    await Promise.all([
      sendOwnerNotification('Contact', formData, metadata),
      sendAutoReply('contact message', formData)
    ]);
  } catch (error) {
